...
uses
    ...
    uAnsTick, aUtil32, inifiles, ScktComp, WinSock,
    Dialogs, Cltype, A2Img, A2Form, Log, mmSystem, unewPackets, Messages;       //ansunit;
...
type
    ...
    TFrmLogOn = class(TForm)
        ...
        procedure ComboBoxHeadChange(Sender: TObject);
        procedure ComboBoxServerChange(Sender: TObject);
        procedure TimePingTimer(Sender: TObject);
    private
        { Private declarations }
  //      clientSendId: integer;
//        clientSendIdtick: integer;
        Floginstate: integer;
...
uses
    ...
    FDepository, filepgkclass, ShellAPI, Fhelp_htm,
    Unit_console;
...
procedure TFrmLogOn.SetNewVersion;
var
    outimg: TA2Image;
    img: TBitmap;
begin
    pgkBmp.getBmp('登陆界面-背景.bmp', A2form.FImageSurface);
    ...
    try
        ...
        PnInfo.Width := 226;
        PnInfo.Height := 16;
        ComboBoxHead.Top := 280;
        ComboBoxHead.Left := 395;

        ComboBoxServer.Left := 395 + 46;
        ComboBoxServer.Top := 280;
       // ComboBox.Width := 139;
       // ComboBox.Height := 16;
        pgkBmp.getBitmap('下拉箭头_按下.bmp', img);
        ComboBoxServer.Picture.Bitmap := img;

        pgkBmp.getBmp('登陆界面-开始游戏-按下.bmp', outimg);
        BtnLogin.A2Down := outimg;
        pgkBmp.getBmp('登陆界面-开始游戏-弹起.bmp', outimg);
        BtnLogin.A2Up := outimg;
        pgkBmp.getBmp('登陆界面-开始游戏-鼠标.bmp', outimg);
        BtnLogin.A2Mouse := outimg;
        pgkBmp.getBmp('登陆界面-开始游戏-禁止.bmp', outimg);
        BtnLogin.A2NotEnabled := outimg;
//        BtnLogin.Transparent := true;
        BtnLogin.Left := 306;
        BtnLogin.Top := 391;
        BtnLogin.Width := 65;
        BtnLogin.Height := 23;

        pgkBmp.getBmp('登陆界面-账号注册-按下.bmp', outimg);
        BtnNewUser.A2Down := outimg;
        pgkBmp.getBmp('登陆界面-账号注册-弹起.bmp', outimg);
        BtnNewUser.A2Up := outimg;
        pgkBmp.getBmp('登陆界面-账号注册-鼠标.bmp', outimg);
        BtnNewUser.A2Mouse := outimg;
        pgkBmp.getBmp('登陆界面-账号注册-禁止.bmp', outimg);
        BtnNewUser.A2NotEnabled := outimg;
//        BtnNewUser.Transparent := true;
        BtnNewUser.Left := 392;
        BtnNewUser.Top := 391;
        BtnNewUser.Width := 65;
        BtnNewUser.Height := 23;

        pgkBmp.getBmp('登陆界面-退出游戏-按下.bmp', outimg);
        BtnExit.A2Down := outimg;
        pgkBmp.getBmp('登陆界面-退出游戏-弹起.bmp', outimg);
        BtnExit.A2Up := outimg;
        pgkBmp.getBmp('登陆界面-退出游戏-鼠标.bmp', outimg);
        BtnExit.A2Mouse := outimg;
        pgkBmp.getBmp('登陆界面-退出游戏-禁止.bmp', outimg);
        BtnExit.A2NotEnabled := outimg;
//        BtnExit.Transparent := true;
        BtnExit.Left := 475;
        BtnExit.Top := 391;
        BtnExit.Width := 65;
        BtnExit.Height := 23;
...
procedure TFrmLogOn.SetOldVersion;
var
    outimg: TA2Image;
begin
    pgkBmp.getBmp('LOGO.bmp', A2form.FImageSurface);

    outimg := TA2Image.Create(32, 32, 0, 0);
    try
        pgkBmp.getBmp('开始游戏DOWN.bmp', outimg);

        BtnLogin.A2Down := outimg;
        pgkBmp.getBmp('开始游戏UP.bmp', outimg);
        BtnLogin.A2Up := outimg;

        pgkBmp.getBmp('账号注册DOWN.bmp', outimg);
        BtnNewUser.A2Down := outimg;
        pgkBmp.getBmp('账号注册UP.bmp', outimg);
        BtnNewUser.A2Up := outimg;

        pgkBmp.getBmp('退出游戏DOWN.bmp', outimg);
        BtnExit.A2Down := outimg;
        pgkBmp.getBmp('退出游戏UP.bmp', outimg);
        BtnExit.A2Up := outimg;
    ...
    { if Loadtimepay then
     begin
         ...
         case TimepayType of
             ...
             Tpt_Close:
                 begin
                     SocketDisConnect(('与服务器连接中断'), ('荤侩磊牢刘'), TRUE);
                 end;
         end;
     end; }
end;
...
procedure TFrmLogOn.sendPack(aid: Integer; aPackType: TPackType);
var
    cCCPack: TCCPack;
begin
...
procedure TFrmLogOn.sendBalance();
var
...
function TFrmLogOn.MessageProcess(var Code: TWordComData): Boolean;             //处理 消息
var
    i: integer;
    str, rdstr: string;
    pckey: PTCKey;
...
{$ENDIF}
    ...
    case pckey^.rmsg of
        ...
        SM_CONNECTTHRU:
            begin
                if FrmNewUser.Visible then
                begin
                    FrmNewUser.MessageProcess(code);
                    exit;
                end;
        ...
        SM_CLOSE:
            begin
                case pckey^.rkey of
                    1: SocketDisConnect(('断开了连接'), ('风云OL'), TRUE);
                    2: SocketDisConnect(('版本错误，下载最新版本。'), ('风云OL'), TRUE, true);
                else
                    begin
                        SocketDisConnect(('断开了连接'), ('风云OL'), TRUE);
                    end;
                end;
...
function TFrmLogOn.ismsgcmd(acmd: integer): boolean;
begin
    Result := false;
...
function TFrmLogOn.SocketAddData(cnt: integer; buf: pointer): Boolean;
var
    n: integer;
    pb: pbyte;
    temp: TWordComData;
...
function TFrmLogOn.SocketDisConnect(ErrText, ErrCaption: string; aActive: Boolean; aopenwww: boolean = false): Boolean;
begin
    Result := true;
    try
...
begin
    ...
    Caption := ('风云OL');
    Pninfo.Caption := ('连接中…若无连线帐号，请按【新名称】建立');
end;
...
begin
    ...
    begin
        ...
        SocketDisConnect(('请不要使用加速度.'), ('风云OL'), TRUE);
 ...
 // end;
    ...
    begin
      ...
      //  TimerNixConnectConfirmed.Enabled := FALSE;
        ...
        SocketDisConnect(('与服务器连接中断'), ('风云OL'), FALSE);
...
procedure TFrmLogOn.WriteLog(aStr: string);
var
    Stream: TFileStream;
    FileName: string;
begin
...
{function (astr:string):string;
begin
    Result := astr;                // ansStringClass.GetString(astr);
end;
}
...
procedure TFrmLogOn.sendIDPASS();
var
    cIdPass: TCIdPass;
begin
...
procedure TFrmLogOn.Timer_loginTimer(Sender: TObject);
begin
    case Floginstate of
        1:                                                                      //连接
            begin
        ...
        2:
            begin
                ...
                begin
                    FGameStatus := gs_BA;
                    sckConnect.Host := ReConnectIpAddr;
        ...
        11:
            begin
                ...
                begin
                    ...
                    sckConnect.Host := ReConnectIpAddr_gate;
...
begin
    ComboBoxServerChange(nil);
 {
    ...
    ConnectState := cs_none;
    sckConnect.Active := false;
    p := HostAddressClass.getSub(ComboBoxHead.Text, ComboBoxServer.ItemIndex);
    if p = nil then exit;



    ReConnectIpAddr := p.rHost;
    ReConnectPort := p.rPort;
...
begin
    //
    FrmNewUser.Visible := true;
    FrmM.SetA2Form(FrmNewUser, FrmNewUser.A2Form);
    PnInfo.Caption := '';
end;
...
procedure TFrmLogOn.ComboBoxHeadChange(Sender: TObject);
begin
    CurServerName := '';
    ComboBoxServer.Text := '';
    ComboBoxServer.Items.Text := HostAddressClass.getSubMenu(ComboBoxHead.Text);
    if ComboBoxServer.Items.Count > 0 then
    begin
...
procedure TFrmLogOn.ComboBoxServerChange(Sender: TObject);
var
    n: integer;
    p: pTHostAddressdata;
begin
    if CurServerName = ComboBoxServer.Text then exit;
    CurServerName := ComboBoxServer.Text;

    ClientIni.WriteString('CLIENT', 'serverhead', ComboBoxHead.Text);
    ClientIni.WriteString('CLIENT', 'servername', ComboBoxServer.Text);
...
procedure TFrmLogOn.TimePingTimer(Sender: TObject);
begin
    sendPing;
end;
