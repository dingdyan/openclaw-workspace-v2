...
uses
  ...
  objcls, CTable, AUtil32, uKeyClass;
...
type
  ...
  public
    ...
    destructor Destroy; override;
    procedure Initialize(ax, ay: integer; aBgEffectShape: integer; aLightEffectKind: TLightEffectKind; boBgOverValue: boolean; astart: integer = 0);
    procedure Finalize;
  ...
  public
    ...
    destructor Destroy; override;
    procedure Clear();
    procedure CreateInitialize(FAtzClass: TAtzClass; ax, ay: integer; aBgEffectShape: integer; aLightEffectKind: TLightEffectKind; boBgOverValue: boolean; astart: integer = 0);
...
//        cImageBufferTemp2: TA2Image;
  ...
  public
    ...
    procedure AddBgEffect(aRealx, aRealy: integer; aShape: integer; aLightEffectKind: TLightEffectKind; boBgOverValue: boolean);
    procedure AddBgEffectList(aRealx, aRealy: integer; aShape: integer; aLightEffectKind: TLightEffectKind; boBgOverValue: boolean);
...
procedure TBgEffectList.CreateInitialize(FAtzClass: TAtzClass; ax, ay: integer; aBgEffectShape: integer; aLightEffectKind: TLightEffectKind; boBgOverValue: boolean; astart: integer = 0);
var
  temp: TBgEffect;
begin
  if aBgEffectShape <= 0 then exit;
  temp := TBgEffect.Create(FAtzClass);
  temp.Initialize(ax, ay, aBgEffectShape, aLightEffectKind, boBgOverValue, astart);
  data.Add(temp);
end;
...
procedure TBgEffect.Initialize(ax, ay: integer; aBgEffectShape: integer; aLightEffectKind: TLightEffectKind; boBgOverValue: boolean; astart: integer = 0);
  ...
  case LightEffectKind of
    lek_none:
      if fboBgOverValue then
        BackScreen.DrawImageOveray(BgEffectImagePP, RealX + ax, RealY + ay, BgOverValue)
      else
...
procedure TCharClass.AddBgEffectList(aRealx, aRealy: integer; aShape: integer; aLightEffectKind: TLightEffectKind; boBgOverValue: boolean);
var
  temp: TBgEffect;
begin
  ...
  BgEffectList.CreateInitialize(FAtzClass, aRealx, aRealy, aShape, aLightEffectKind, boBgOverValue);

end;

procedure TCharClass.AddBgEffect(aRealx, aRealy: integer; aShape: integer; aLightEffectKind: TLightEffectKind; boBgOverValue: boolean);
begin
  case aLightEffectKind of
    lek_none, lek_follow, lek_future:
      begin
        BgEffect.Initialize(aRealx, aRealy, aShape, aLightEffectKind, boBgOverValue);
      end;
    lek_cumulate, lek_cumulate_follow:
      begin
        AddBgEffectList(aRealx, aRealy, aShape, aLightEffectKind, boBgOverValue);
      end;
  end;
      ...
      hs_100: // 老馆利牢 葛嚼
        begin
          ...
          if cImageBuffer_w <> nil then
            BackScreen.DrawImageAdd(cImageBuffer_w, RealX - CHarMaxSiezHalf - caddx, RealY - CHarMaxSiezHalf - caddy);
...
begin
  ...
  begin
    ...
    if FrmGameToolsNew.A2CheckBox_ShowItem.Checked then
    begin
      if TDynamicObject(DynamicObjDataList[i]).id = SelectedDynamicItem then
        Col := NameChangeColor
      else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (FrmGameToolsNew.A2CheckBox_ShowMonster.Checked) then
        begin
          if cl.id = SelectedChar then
            Col := NameChangeColor
          else
      ...
      begin
        ...
        begin
          ...
          str := cl.rName;
          if cl.GuildName <> '' then str := str + ',' + cl.GuildName;
          BackScreen.DrawName(aCanvas, Cl.RealX, Cl.RealY - 40, str, Col);
        end;

        if FrmGameToolsNew.A2CheckBox_ShowNpc.Checked then
        begin
          if cl.id = SelectedChar then
            Col := NameChangeColor
          else
      ...
      begin
        ...
        begin
          ...
          begin
            if FrmGameToolsNew.A2CheckBox_ShowPlayer.Checked then
            begin
              if cl.id = SelectedChar then
              begin
                col := NameChangeColor;
    ...
    begin
      ...
      begin
        if cl.GuildName <> '' then
          BackScreen.DrawBubble(aCanvas, Cl.RealX, Cl.RealY - 40, Cl.BubbleList)
        else
          BackScreen.DrawBubble(aCanvas, Cl.RealX, Cl.RealY - 20, Cl.BubbleList);
      end
      else
      begin
