...
uses FMain, FAttrib, FLogon, FBottom, filepgkclass, Math, FWearItem;
        ...
        case PSSShowSpecialWindow^.rWindow of
          ...
          WINDOW_ITEMLOG:
            begin
              ...
              if FrmWearItem.Visible = false then FrmBottom.ButtonWearClick(nil);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            SetItemLabel(LabelArr[rkey], (aitem.rViewName), aitem.rColor, aitem.rshape, idx, 0);
...
begin
  Lb.A2ImageRDown := nil;
  Lb.A2ImageLUP := nil;
  Lb.A2Image := ItemLogBackImg;
  Lb.A2Imageback := ItemLogBackImg;
...
begin
  ...
  begin
    ...
    if shapeRdown <> 0 then Lb.A2ImageRDown := savekeyImagelib[shapeRdown];
  end;
    {
if shape = 0 then
begin
...
begin
  ...
  TEMPFrmQuantity.EdCount.Text := inttostr(TEMPFrmQuantity.CountCur);
...
begin
  ...
  if TDragItem(Source).SourceID <> WINDOW_ITEMS then exit;
...
begin
    { Top := (600 - 117 - Height) div 2;
     if FrmAttrib.Visible then Left := (800 - 180 - Width) div 2
     else Left := (800 - Width) div 2;
     }
end;
...
begin
  if FrmAttrib.Visible then Result := 800 - FrmAttrib.Width
  else Result := 800;
end;
...
begin
  if FrmBottom.Visible then Result := (600 - Frmbottom.Height) + 10
  else Result := 600;
end;
...
procedure TFrmDepository.FormMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  GameHint.Close;
end;
