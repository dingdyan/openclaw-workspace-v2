...
uses
    SysUtils, Classes, DefType, AUtil32, uAnsTick, PaxScripter, svClass;
...
uses
    svMain, UserSDB, MapUnit, FieldMsg, uMonster, uNpc, BasicObj,
    uGuild, uUser, uDoorGen, FSockets;
...
begin
    ...
    //没有配置 移动到 长城以南 500 500
        UserList.MoveByServerID(ServerID, 1, 500, 500);
    end;
    if ItemList <> nil then TItemList(ItemList).AllClear;
    if MonsterList <> nil then TMonsterList(MonsterList).ReLoad;
    if NpcList <> nil then TNpcList(NpcList).ReLoad;
    if DynamicObjectList <> nil then TDynamicObjectList(DynamicObjectList).ReLoad;
