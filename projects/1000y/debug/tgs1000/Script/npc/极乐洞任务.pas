...
const
    //任务1信息
    C_p_Age         = 100;        //限制条件 年龄
    C_p_Energy      = 4;           //限制条件 元气境界
    C_p_ID          = 8006;        //任务ID
    C_p_ItemPre     = '令牌8006';  //前驱任务品
    C_p_Item        = '头颅8006';  //任务物品
    C_p_ItemPreview = '缉拿令牌';  //前驱任务品
    C_p_Itemview    = '亡灵童子的头颅'; //任务物品
    C_p_ItemSum     = 15;          //任务物品个数
    C_p_Reward      = 60;          //任务奖励
    C_p_Multiple    = 2;           //奖励倍数
    C_p_MenuName    = '剿灭亡灵童子'; //任务名字
    C_p_Des         = '一群可恶的亡灵童子，目无王法，鱼肉百姓。^'
        + '你能帮我去消灭他们吗？'; //接任务描述
    C_p_Des_end     = '那么快就回来了，消灭他们了吗？'; //交任务描述
    C_p_Des_Help    = '任务要求：30岁以上，生死境或更高^'
        + '任务奖励：荣誉^'
        + '任务规则：该任务每天只能重复做10次，^'
        + '重复次数越多，奖励越多^'; //任务帮助
    //任务2信息
    C_x_Energy      = 4;           //限制条件 元气境界
    C_x_ID          = 8206;        //任务ID
    C_x_ItemPre     = '令牌8206';  //前驱任务品
    C_x_Item        = '头颅8206';  //任务物品
    C_x_ItemPreview = '缉拿令牌';  //前驱任务品
    C_x_Itemview    = '亡灵童子的头颅'; //任务物品
    C_x_ItemSum     = 15;          //任务物品个数
    C_x_Reward      = 10000000;    //任务奖励
    C_x_Multiple    = 2;           //奖励倍数
    C_x_MenuName    = '剿灭亡灵童子2'; //任务名字
    C_x_Des         = '一群可恶的亡灵童子，目无王法，鱼肉百姓。^'
        + '你能帮我去消灭他们吗？'; //接任务描述
    C_x_Des_end     = '那么快就回来了，消灭他们了吗？'; //交任务描述
    C_x_Des_help    = '任务要求：30岁以上，生死境或更高^'
        + '任务奖励：武功经验^'
        + '任务规则：该任务每天只能重复做10次，^'
        + '重复次数越多，奖励越多^'; //任务帮助

    C_m_Energy      = 4;           //限制条件 元气境界
    C_m_ID          = 8106;        //任务ID
    C_m_ItemPre     = '令牌8106';  //前驱任务品
    C_m_Item        = '首级8106';  //任务物品
    C_m_ItemPreview = '缉拿令牌';
    C_m_Itemview    = '九法僧首级';
    C_m_ItemSum     = 9;           //任务物品个数
    C_m_Reward      = 70000;       //任务奖励
    C_m_Multiple    = 2;           //奖励倍数
    C_m_MenuName    = '九法僧之死'; //任务名字
    C_m_Des         = '为了让极乐洞成为真正的极乐洞，九法僧的存在^已成为阻碍'; //接任务描述
    C_m_Des_end     = '我交给你的任务完成得怎么样了？'; //交任务描述
    C_m_Des_help    = '任务要求：30岁以上，生死境或更高^'
        + '任务奖励：钱币^'
        + '任务规则：该任务每天只能重复做10次，^'
        + '重复次数越多，奖励越多^'; //任务帮助
