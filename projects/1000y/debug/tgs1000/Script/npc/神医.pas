...
begin
    Menusay(uSource, '专业炼丹师 神医 需要帮助吗?^^'
        + '     <〖成为炼丹师〗/@notice>^' //chemist
        + '     <〖转职为炼丹师〗/@notice>^' //reqchange
        + '     <〖什么是炼丹术〗/@jobhelp>^'
        + '     <〖询问有关加工事宜〗/@processhelp>^'
        + '     <〖职业进阶〗/@notice>^' //jobLevel
        + '     <〖 任 务 〗/@Quest>^');
end;

procedure notice(uSource, uDest:integer);
begin
    Menusay(uSOurce, '此功能暂时未开放！^^'
        + '<〖返回〗/@OnMenu>^^'
        + '<〖退出〗/@exit>');
end;
    ...
    case CurQuestId of
        4950:
            begin
                ...
                if CurQuestStep = 18 then
                begin
                    Menusay(uSource, '很高兴认识你啊，我这儿不仅收购出售各种药^'
                        + '品，还可以学习炼丹技术，需要的话随时可以^'
...
procedure q4950_j19(uSource, uDest:integer);
var
    aname, notice   :string;
    acount          :integer;
begin
    ...
    notice := getQuestSubRequest(4950, 19);
    saysystem(uSource, '任务提示：' + notice);

    if getQuestSubItem(4950, 18, 0, aname, acount) = false then exit;
    additem(uSource, aname, acount);

    if getQuestSubItem(4950, 18, 1, aname, acount) = false then exit;
    additem(uSource, aname, acount);
...
begin
    //判断是否为炼丹师
    if getJobKind(uSource) <> 2 then
    begin
        Menusay(uSource, '您不是炼丹师,不能在我这里选择进阶!^^'
            + '<〖返回〗/@OnMenu>^^'
            + '<〖退出〗/@exit>');
        exit;
    end;
    ...
    menusay(uSource, '只有不断进阶，才能最终走向成功之路!^'
        + '你现在的是炼丹师（' + getJobGradeName(uSource) + '）^'
...
begin
    ...
    Menusay(uSource, '恭喜你，你现在是炼丹师（' + getJobGradeName(uSource) + '）了!^^'
        + '<〖返回〗/@OnMenu>^^'
        + '<〖退出〗/@exit>');
end;

procedure chemist(uSource, uDest:integer);
var
    i               :integer;
begin
    ...
    if i = 2 then
    begin
        menusay(uSource, '您已经是炼丹师了^^'
            + '<〖返回〗/@OnMenu>^^'
            + '<〖退出〗/@exit>');
        exit;
    end;

    menusay(uSource, '成为炼丹师需要[钱币]30000个，你确定^'
        + '要当炼丹师吗^^'
        + '<〖确定〗/@alchemist_OK>^^'
        + '<〖返回〗/@OnMenu>^^'
        + '<〖退出〗/@exit>');
...
begin
    ...
    setjobKind(uSource, 2);
    menusay(uSource, '恭喜你，你现在是炼丹师了^^'
        + '<〖返回〗/@OnMenu>^^'
        + '<〖退出〗/@exit>');
end;
...
begin
    ...
    if i = 2 then
    begin
        menusay(uSource, '你本来就是炼丹师^^'
            + '<〖返回〗/@OnMenu>^^'
            + '<〖退出〗/@exit>');
        exit;
    end;

    menusay(uSource, '转职为炼丹师需要[钱币]50000个^'
        + '技能等级还原到1.00级^'
        + '你确定转职吗?^^'
        + '<〖确定〗/@change>^^'
        + '<〖返回〗/@OnMenu>^^'
...
begin
    Menusay(uSource, '炼丹术是制造多种试剂的技术,炼丹师用各种^'
        + '草药与决定其作用的药材 作各种试剂^^'
        + '<〖返回〗/@OnMenu>^^'
        + '<〖退出〗/@exit>');
end;

procedure processhelp(uSource, uDest:integer);
begin
    Menusay(uSource, ' 炼丹师加工处的试剂能将武器,衣服,饰品的^'
        + '潜在力发挥到最大加工不太容易,但是当他的^'
        + '能力值足够制作辅助试剂时,加工将容易许多^^'
        + '<〖返回〗/@OnMenu>^^'
        + '<〖退出〗/@exit>');
end;
