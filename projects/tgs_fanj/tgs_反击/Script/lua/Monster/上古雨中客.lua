...
[[幸会. 我是肩负守护王陵重任的雨中客. ^
现今夜贼频出,不知侠客是否愿拔刀相助.^^
<「游标.bmp」『$00FFFF00| 接受委托.』/@request>^
<「游标.bmp」『$00FFFF00| 铲除了忍王一伙儿』/@removal>^
<「游标.bmp」『$00FFFF00| 取王陵四大宝物拿给雨中客』/@returntreasure>^
<「游标.bmp」『$00FFFF00| 请雨中客转送至禁区.』/@goarea>^
]];

--死亡
function OnDie(uSource, uDest, race)
  --查找地图中怪物是否存在
  if M_MapFindMonster(70, '上古雨中客2', 0) > 0 then
    M_MapDelMonster(70, '上古雨中客2');
  end
  M_MapAddMonster(70, '上古雨中客2', 178, 176, 1, 1, '', 0, 0, true, 0);
  return;
end;
...
function OnGetResult(uSource, uDest, aStr)
  ...
  if aStr == 'goarea' then
    --获取地图剩余时间 
    local iTick = M_GetMapRemainTickById(70)
	if iTick <= 30000 then
    ...
    --检测地图玩家数
    if M_MapUserCount(76) > 0 then
      P_MenuSay(uSource, '稍等._禁区好像有人...');
     return;
    end;
    --刷新地图
    if not M_MapRegen(76) then 
      P_MenuSay(uSource, '稍等片刻...');
     return;
    end
    --检测怪物
    if M_MapFindMonster(76, '禁地护卫武士', 0) <= 0 then 
    ...
    --1秒后传送
	P_MapMove(uSource, 76, 14, 21, 100);
    --NPC说话
