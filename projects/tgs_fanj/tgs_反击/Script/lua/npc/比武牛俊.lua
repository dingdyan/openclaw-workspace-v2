...
function OnCreate(uDest, uSource, aStr)
  ...
  B_SayDelayAdd(uDest, '如果10秒内不能将你搞定', 50); 
  B_SayDelayAdd(uDest, '就算我输!', 400); 
  return;
end;


function OnChangeState(uSource, uDest, uState)
  B_SayDelayAdd(uDest, '领教了吧!', 50); 
  B_SayDelayAdd(uDest, '差远了...', 400); 

  if P_getitemspace(uSource) < 1 then
    B_SayDelayAdd(uDest, '物品栏已满...', 100); 
  else
	P_additem(uSource, '生药', 10, '比武梅花夫人');
  end;
...
function OnDie(uSource, uDest, uRace)
  ...
  B_SAY(uDest, '服不服!_还不服气!!'); 
  M_MapRegen(58);
  P_MapMove(uSource, 58, 20, 18, 500);
