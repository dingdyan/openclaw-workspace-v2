...
function OnCreate(uDest, uSource, aStr)
  ...
  if P_CheckPowerWearItem(uSource) > 0 then 
    ...
    P_MapMove(uSource, 1, 599, 182, 100);
    return;
  end;
  ...
  if GradeUp ~= 1 then 
    B_SayDelayAdd(uDest, string.format('%s不是2级绝世武功', CurMagic), 0);
    P_MapMove(uSource, 1, 599, 182, 100);
    return;
  end
...
function OnChangeState(uSource, uDest, uState)
  ...
  P_MapMove(uSource, 1, 599, 182, 600);
 return;
end
...
function OnDie(uSource, uDest, uRace)
  ...
  P_MapMove(uSource, 1, 599, 182, 500);
  P_SetUseMagicGradeUp(uSource, 1, 2);
  --检测是否第一次 给真气
  if P_GetTempArr(uSource, 16) == 1 then 
    P_SetTempArr(uSource, 16, 2);
    --给予真气
    P_SetAddableStatePoint(uSource, P_GetAddableStatePoint(uSource) + 200);
    P_LeftText(uSource, string.format('真气 获得 %d 个', 200), 55769, 3);
  end 
  --公告
  M_topmsg(string.format('%s 恭喜你,%s升到了3级', B_GetRealName(uSource), CurMagic), 65280);
  M_SetMapEnter(85, true);
 return;
end
