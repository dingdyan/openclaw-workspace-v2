...
function OnCreate(uDest, uSource, aStr)
  ...
  B_SayDelayAdd(uDest, '领教了', 50); 
  B_SayDelayAdd(uDest, '开始吧', 400); 
  return;
end;


function OnChangeState(uSource, uDest, uState)
  B_SayDelayAdd(uDest, '不错。只要能再坚持几秒钟', 50); 
  B_SayDelayAdd(uDest, '到那时你将无敌于天下', 400); 

  if P_getitemspace(uSource) < 1 then
    B_SayDelayAdd(uDest, '物品栏已满...', 100); 
  else
	P_additem(uSource, '生药', 20, '比武牛俊');
  end;
...
function OnDie(uSource, uDest, uRace)
  ...
  B_SAY(uDest, '意料之中_真让在下过足了眼瘾'); 
  M_MapRegen(59);
  P_MapMove(uSource, 59, 20, 18, 500);
