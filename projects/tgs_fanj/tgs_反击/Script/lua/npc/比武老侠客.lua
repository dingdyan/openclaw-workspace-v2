...
function OnCreate(uDest, uSource, aStr)
  ...
  B_SayDelayAdd(uDest, '如果你实在想比', 50); 
  B_SayDelayAdd(uDest, '我将奉陪到底', 400); 
  return;
end;


function OnChangeState(uSource, uDest, uState)
  B_SayDelayAdd(uDest, '小小年纪_就有如此高深的武功', 50); 
  B_SayDelayAdd(uDest, '日后要继续修炼...', 400); 

  if P_getitemspace(uSource) < 1 then
    B_SayDelayAdd(uDest, '物品栏已满...', 100); 
  else
	P_additem(uSource, '罗刹卷轴', 1, '比武忍王');
  end;
...
function OnDie(uSource, uDest, uRace)
  ...
  B_SAY(uDest, '恭喜恭喜_让老朽佩服不已'); 
  M_MapRegen(61);
  P_MapMove(uSource, 61, 20, 18, 500);
