...
function OnGetResult(uSource, uDest, aStr)
 ...
 if aStr == 'xsrw' then
	...
	if Value == 4 then 
	  --任务流程==4,分配任务ID 5,提示让去杀怪物
	  P_SetTempArr(uSource, NewTask.QuestArr, 5);
	  P_SetTempArr(uSource, NewTask.DieArr, 0);
	  --任务提示
	  P_MenuSay(uSource, '领取到一个新的任务,' .. NewTask.Info[5]);
	...
	if Value == 5 then 
	  ...
	  if PlayDie < NewTask.Kill[Value] then 
        P_MenuSay(uSource, '需要击杀1只鹿,任务尚未完成,请继续努力');
       return;	
	  end	
	  --给予奖励
