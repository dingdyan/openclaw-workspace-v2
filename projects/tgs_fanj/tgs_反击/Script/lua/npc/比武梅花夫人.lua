...
function OnCreate(uDest, uSource, aStr)
  ...
  B_SayDelayAdd(uDest, '别害怕', 50); 
  B_SayDelayAdd(uDest, '我会手下留情的...', 400); 
  return;
end;


function OnChangeState(uSource, uDest, uState)
  B_SayDelayAdd(uDest, '别无他法', 50); 
  B_SayDelayAdd(uDest, '是不是修炼方法不妥呀?', 400); 

  if P_getitemspace(uSource) < 1 then
    B_SayDelayAdd(uDest, '物品栏已满...', 100); 
  else
	P_additem(uSource, '生药', 5, '比武捕盗大将');
  end;
...
function OnDie(uSource, uDest, uRace)
  ...
  B_SAY(uDest, '我可只用了3成功力'); 
  M_MapRegen(57);
  P_MapMove(uSource, 57, 20, 18, 500);
