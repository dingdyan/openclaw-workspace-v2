...
function OnItemDblClick(uSource, uItemKey, astr)
   ...
   --检测道具数量
   if P_getitemcount(uSource, '经验果1') < 1 then 
	 P_saysystem(uSource, '没有经验果(绑定)', 2);
     return;
   end
   --获取玩家名称
   ...
   --删除道具
   P_deleteitem(uSource, '经验果1', 1, '双倍卷轴');
   P_saysystem(uSource, '增加1小时双倍经验,使用口令[@双倍经验情报]可查询剩余时间.', 2);
 return;
end