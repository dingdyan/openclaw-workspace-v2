...
--[[  if arg[1] == '@反击开启' then
  ...
  --开启VIP属性
  if arg[1] == '@开启VIP属性' then
    VipAttrib.open(uSource);
   return 'true';
  end;
  --开启VIP属性
  if arg[1] == '@关闭VIP属性' then
...
function OnKilled(uSource, ...)
  ...
  local MapId, X, Y =  B_GetPosition(uSource);
    ...
    --判断身上装备
	local ItemData = P_GetWearItemInfoTabs(uSource, 13);
	if ItemData.Name == '武侠令(30天)' or ItemData.Name == '武侠令(3天)' then 
   ...
   --检测是否是VIP
    ...
    if viplevel < 3 then 	
	 P_MapMove(uSource, 2, 65 + math.random(-6, 6), 56 + math.random(-6, 6), 0);
	 P_saysystem(uSource, 'VIP等级未到达3级或者武侠令没有佩戴,死亡将你传送至大战场', 2);
    end;
   end;
  end;
 end;
