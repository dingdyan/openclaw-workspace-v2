...
uses
  ...
  uKeyClass, uGramerID, PaxScripter, PaxPascal, DateUtils, uMagicClass;
...
type
  ...
  public
    ...
    function IsScripter(JName: string): boolean;
  ...
  TPowerLevelSub = class
    LifeDataarr: array[1..13] of TPowerLeveldata;
  public
    constructor Create;
    destructor Destroy; override;
    procedure Clear();
...
var
  i: integer;
begin
  NEWItemIDClass.ItemNewId(aitem);
...
begin
  ...
  if not isFullHangul(iName)
        //or not isGrammarID(iName)
  then
  begin
    aRetStr := '无法使用的门派武功名';
    exit;
  ...
  if (MagicData.rEventDecMagic < 5) or (MagicData.rEventDecMagic > 35) then
  begin
    aRetStr := '武功消耗只允许5-35的值';
    exit;
  end;
...
begin
  ...
  if RandomClass.GetChance(aCheckItem.rName, aObjName) = false then
  begin
    FillChar(ItemData, sizeof(ItemData), 0);
    exit;
  end;
...
arr_weapon(武器)
  ...
  if (str <> '') and ItemClass.GetItemData(str, aitem) then
  begin
...
begin
  if SaveTick + 5 * 60 * 100 <= CurTick then
  begin
    SaveTick := CurTick;
    SaveToFile('Prison.SDB');
  end;
...
begin
  fillchar(LifeDataarr, sizeof(TPowerLeveldata) * (high(LifeDataarr) + 1), 0);
end;
