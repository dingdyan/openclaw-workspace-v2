...
uses
    ...
    A2Form, A2Img, Deftype, ExtCtrls, StdCtrls, CharCls, cltype, AtzCls,
    uAnsTick, BackScrn, Gauges, Menus, jpeg, uMagicClass, Usersdb, uKeyClass;   //Fdyeing,
...
type
    ...
    TKeyClassDataKey = (kcdk_none, kcdk_HaveItem, kcdk_HaveMagic, kcdk_BasicMagic
        , kcdk_HaveRiseMagic, kcdk_HaveMysteryMagic);
    ...
    A2Label1: TA2Label;
        ...
        procedure BLabel0MouseDown(Sender: TObject; Button: TMouseButton;
            Shift: TShiftState; X, Y: Integer);
        ...
        procedure listContentKeyUp(Sender: TObject; var Key: Word;
            Shift: TShiftState);
...
uses FMain, FBottom, FLogOn, FDepository, FQuantity, FNPCTrade,
    FCharAttrib, FItemHelp, FConfirmDialog, FShowPopMsg, FWearItem,
    FWearItemUser, filepgkclass, FnewMagic, uPersonBat;
...
begin
    Result := nil;
    if (idx < 0) or (idx >= FrmAttrib.WWWImageLib.Count) then exit;
...
begin
    Result := nil;
    if (idx < 0) or (idx >= FrmAttrib.viewImage.Count) then exit;
...
begin
    Result := nil;
    if (idx < 0) or (idx >= FrmAttrib.energyImageLib.Count) then exit;
...
begin
    ...
    begin
        if ItemArr[akey].rboNotTrade = false then result := true;
    end;
end;
...
begin
    ...
    if (akey >= low(ItemArr)) and (akey <= high(ItemArr)) then
    begin
        if ItemArr[akey].rboNotExchange = false then result := true;
    end;
end;
...
begin
    ...
    if (akey >= low(ItemArr)) and (akey <= high(ItemArr)) then
    begin
        if ItemArr[akey].rboNotDrop = false then result := true;
    end;
end;
...
begin
    ...
    if (akey >= low(ItemArr)) and (akey <= high(ItemArr)) then
    begin
        if ItemArr[akey].rboNotSSamzie = false then result := true;
    end;
end;
...
begin
    ...
    begin
        ...
        if assigned(FonUPdate) then FonUPdate(akey, ItemArr[akey]);
    end;
end;
...
begin
    ...
    begin
        ...
        if aitem.rName <> '' then
        begin
            if cItemTextClass.get(aitem.rName) = nil then sendItemText(aitem.rName);
        end;
        if assigned(FonUPdate) then FonUPdate(akey, ItemArr[akey]);
    end;
end;
...
begin
    if (akey >= low(ItemArr)) and (akey <= high(ItemArr)) then
        ...
        begin
            ...
            ItemArr[akey2] := m;
            if assigned(FonUPdate) then FonUPdate(akey, ItemArr[akey]);
            if assigned(FonUPdate) then FonUPdate(akey2, ItemArr[akey2]);
        end;
end;
...
begin
    ...
    if Assigned(onhead) then onhead(CurAttribData.CurheadSeak, AttribData.cheadSeak);
end;

procedure TcAttribClass.setLeg(Value: integer);
begin
    AttribData.cLegSeak := Value;
    if Assigned(onleg) then onleg(CurAttribData.CurLegSeak, AttribData.cLegSeak);
end;
...
begin
    ...
    if Assigned(onArm) then onArm(CurAttribData.CurarmSeak, AttribData.cArmSeak);
end;
                ...
                case pSAttribUPDATE.rType of
                    aut_rprestige:
                        begin
                            ...
                            if Assigned(onprestige) then onprestige(AttribData.prestige);
                        end;
                    aut_rprestige_Add:
                        begin
                            AttribData.prestige := AttribData.prestige + pSAttribUPDATE.rvaluer;
                            if Assigned(onprestige) then onprestige(AttribData.prestige);
                            if Assigned(onprestigeadd) then onprestigeadd(pSAttribUPDATE.rvaluer);
...
begin
    HaveMagic := TcMagicListClass.Create(30);
    DefaultMagic := TcMagicListClass.Create(20);
    HaveRiseMagic := TcMagicListClass.Create(30);
    HaveMysteryMagic := TcMagicListClass.Create(30);
end;
...
//        FrmBottom.ShortcutKeyUP(akey + 60);
        if aitem.rcount <= 1 then ILabels[akey].Hint := (aitem.rViewName)
        else ILabels[akey].Hint := (aitem.rViewName) + ':' + IntToStr(aitem.rCount);
    end;
...
begin
  ...
  //  pgkBmp.getBitmap('attrib.bmp', Image1.Picture.Bitmap);
    ...
    for i := 0 to 30 - 1 do SetMagicLabel(MLabels[i], '', 0, 0, 0);
    for i := 0 to 30 - 1 do SetMagicLabel(BLabels[i], '', 0, 0, 0);
//    for i := 0 to high(BasicBufferkey) do BasicBufferkey[i] := 255;

    for i := 0 to 30 - 1 do MLabels[i].A2ShowHint := FALSE;
    for i := 0 to 30 - 1 do BLabels[i].A2ShowHint := FALSE;
    for i := 0 to 30 - 1 do
    begin
        ...
        SM_ITEM_UPDATE:
            begin
                ...
                case TSENDUPDATEITEMTYPE(nkey) of
                    suitHave: pitem := HaveItemclass.getid(idx);
                    suitWear: pitem := WearItemclass.wear.getid(idx);
                    suitWearFd: pitem := WearItemclass.wearFD.getid(idx);
                else exit;
                end;
                n := WordComData_GETbyte(code, i);
                case n of
                    ITEM_UPDATE_add:
                        begin
                            ...
                            case TSENDUPDATEITEMTYPE(nkey) of
                                ...
                                suitWear: WearItemclass.Wear.upitem(idx, aitem);
                                suitWearFd: WearItemclass.wearFD.upitem(idx, aitem);
                            else exit;
                            end;
                        end;
                    ITEM_UPDATE_del:
                        begin
                            case TSENDUPDATEITEMTYPE(nkey) of
                                suitHave: HaveItemclass.del(idx);
                                suitWear: WearItemclass.Wear.del(idx);
                                suitWearFd: WearItemclass.wearFD.del(idx);
                            else exit;
                            end;
                        end;
                    ITEM_UPDATE_rcolor:
                        begin
                            ...
                            case TSENDUPDATEITEMTYPE(nkey) of
                                ...
                                suitWear: WearItemclass.Wear.upitemcolor(idx, n1);
                                suitWearFd: WearItemclass.wearFD.upitemcolor(idx, n1);
                            else exit;
                            end;
                        end;
                    ITEM_UPDATE_ChangeItem:
                        begin
                            ...
                            case TSENDUPDATEITEMTYPE(nkey) of
                                suitHave: HaveItemclass.upitemChangeItem(idx, n1);
                            else exit;
                            end;
                        end;
                    ITEM_UPDATE_rcount_add:
                        begin
                            ...
                            case TSENDUPDATEITEMTYPE(nkey) of
                                ...
                                suitWear: WearItemclass.Wear.upitemCountadd(idx, n1);
                                suitWearFd: WearItemclass.wearFD.upitemCountadd(idx, n1);
                            else exit;
                            end;
                        end;
                    ...
                    ITEM_UPDATE_rcount_UP:
                        begin
                            ...
                            case TSENDUPDATEITEMTYPE(nkey) of
                                ...
                                suitWear: WearItemclass.Wear.upitemCountUP(idx, n1);
                                suitWearFd: WearItemclass.wearFD.upitemCountUP(idx, n1);
                            else exit;
                            end;
                        end;
                    ITEM_UPDATE_rcount_dec:
                        begin
                            n1 := WordComData_GETdword(code, i);
                            case TSENDUPDATEITEMTYPE(nkey) of
                                suitHave: HaveItemclass.upitemCountdec(idx, n1);
                                suitWear: WearItemclass.Wear.upitemCountdec(idx, n1);
                                suitWearFd: WearItemclass.wearFD.upitemCountdec(idx, n1);
                            else exit;
                            end;
                        end;
                    ...
                    ITEM_UPDATE_rlockState:
                        begin
                            ...
                            case TSENDUPDATEITEMTYPE(nkey) of
                                suitHave: HaveItemclass.upitemLockState(idx, n1);
                                suitWear: WearItemclass.Wear.upitemLockState(idx, n1);
                                suitWearFd: WearItemclass.wearFD.upitemLockState(idx, n1);
                            else exit;
                            end;
                        end;
                    ...
                    ITEM_UPDATE_rboident:
                        begin
                            n1 := WordComData_GETbyte(code, i);
                            case TSENDUPDATEITEMTYPE(nkey) of
                                suitHave: HaveItemclass.upitemBoIdent(idx, boolean(n1));
                                suitWear: WearItemclass.Wear.upitemBoIdent(idx, boolean(n1));
                                suitWearFd: WearItemclass.wearFD.upitemBoIdent(idx, boolean(n1));
                            else exit;
                            end;
                        end;
                    ...
                    ITEM_UPDATE_rboBlueprint:
                        begin
                            n1 := WordComData_GETbyte(code, i);
                            case TSENDUPDATEITEMTYPE(nkey) of
                                suitHave: HaveItemclass.upitemboBlueprint(idx, boolean(n1));
                                suitWear: WearItemclass.Wear.upitemboBlueprint(idx, boolean(n1));
                                suitWearFd: WearItemclass.wearFD.upitemboBlueprint(idx, boolean(n1));
                            else exit;
                            end;
                        end;
                end;
                ...
                begin
                  ...
                  //  keysetImg(idx, pskey.rKEY[idx]);
                    case pskey.rKEY[idx] of
                        0..29: cKeyClass.UPdate(idx, kcdt_F5_f12, kcdk_BasicMagic, pskey.rKEY[idx]);
                        30..59: cKeyClass.UPdate(idx, kcdt_F5_f12, kcdk_HaveMagic, pskey.rKEY[idx] - 30);
                        60..89: cKeyClass.UPdate(idx, kcdt_F5_f12, kcdk_HaveItem, pskey.rKEY[idx] - 60);
                        90..119: cKeyClass.UPdate(idx, kcdt_F5_f12, kcdk_HaveRiseMagic, pskey.rKEY[idx] - 90);
                        120..149: cKeyClass.UPdate(idx, kcdt_F5_f12, kcdk_HaveMysteryMagic, pskey.rKEY[idx] - 120);
                    end;
                end;
                ...
                begin
                   // FrmBottom.ShortcutKeySETimg(idx, pskey.rKEY2[idx]);
                    case pskey.rKEY2[idx] of
                        0..29: cF5_F12Class.UPdate(idx, kcdt_key, kcdk_BasicMagic, pskey.rKEY2[idx]);
                        30..59: cF5_F12Class.UPdate(idx, kcdt_key, kcdk_HaveMagic, pskey.rKEY2[idx] - 30);
                        60..89: cF5_F12Class.UPdate(idx, kcdt_key, kcdk_HaveItem, pskey.rKEY2[idx] - 60);
                        90..119: cF5_F12Class.UPdate(idx, kcdt_key, kcdk_HaveRiseMagic, pskey.rKEY2[idx] - 90);
                        120..149: cF5_F12Class.UPdate(idx, kcdt_key, kcdk_HaveMysteryMagic, pskey.rKEY2[idx] - 120);
                    end;
                end;
...
begin
    ...
    begin
        ...
        begin
            ...
            frmConfirmDialog.ShowFrom(cdtItemStirng, '输入文字', '');
            exit;
        end;
        //未做限制 发送间隔 时间
        cClick.rmsg := CM_DBLCLICK;
        ...
        case SourceID of
            WINDOW_ITEMLOG:
                begin
                    ...
                    FrmDepository.showinputCountOUt(
                        DragItem.Selected, TA2ILabel(Sender).tag,
                        1,
                        FrmDepository.get(DragItem.Selected).rCount,
                        FrmDepository.get(DragItem.Selected).rViewName);
                    exit;
                end;
...
begin
    ...
    begin
        ...
        FITEMA2Hint.Fleft := 800 - 180 - FITEMA2Hint.Fback.Width - 8;
...
begin
    BackScreen.SWidth := 800 - 192;
    // BackScreen.SHeight := 600 - 117;
     //   FrmM.DXTimerTimer(Self, 0);
end;

procedure TFrmAttrib.FormHide(Sender: TObject);
begin
    BackScreen.SWidth := 800;
    //BackScreen.SHeight := 600 - 117;
    //   FrmM.DXTimerTimer(Self, 0);
end;
...
procedure TFrmAttrib.A2ILabel0MouseDown(Sender: TObject;
    Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
begin
    ...
    begin
        if (((PanelItem.Visible or PanelMagic.Visible or PanelBasic.Visible) and FrmAttrib.Visible) = false)
            and (FrmNewMagic.Visible = false)
            and (FrmWearItem.Visible = false) then exit;
        if keyselmagicindexadd = -1 then exit;
        case keyselmagicindexadd of
            0..29: cF5_F12Class.UPdate(id, kcdt_F5_f12, kcdk_BasicMagic, keyselmagicindexadd);
            30..59: cF5_F12Class.UPdate(id, kcdt_F5_f12, kcdk_HaveMagic, keyselmagicindexadd - 30);
            60..89: cF5_F12Class.UPdate(id, kcdt_F5_f12, kcdk_HaveItem, keyselmagicindexadd - 60);
            90..119: cF5_F12Class.UPdate(id, kcdt_F5_f12, kcdk_HaveRiseMagic, keyselmagicindexadd - 90);
            120..149: cF5_F12Class.UPdate(id, kcdt_F5_f12, kcdk_HaveMysteryMagic, keyselmagicindexadd - 120);
        end;
//        keysetsave(id, keyselmagicindexadd);
    ...
    begin
        ...
        if (p.rkey < 0) or (p.rkey > 30) then exit;
        case p.rkeyType of
            kcdk_BasicMagic: FrmNewMagic.BLabels[p.rkey].OnDblClick(BLabels[p.rkey]);
            kcdk_HaveMagic: FrmNewMagic.MLabels[p.rkey].OnDblClick(MLabels[p.rkey]);
            kcdk_HaveItem: FrmWearItem.ILabels[p.rkey].OnDblClick(ILabels[p.rkey]);
            kcdk_HaveRiseMagic: FrmNewMagic.RiseLabels[p.rkey].OnDblClick(MLabels[p.rkey]);
            kcdk_HaveMysteryMagic: FrmNewMagic.MysteryLabels[p.rkey].OnDblClick(MLabels[p.rkey]);
        end;
    end;
end;
...
begin
    result := nil;
    case id of
        0..29: result := FrmNewMagic.BLabels[id];
        30..59: result := FrmNewMagic.MLabels[id - 30];
        60..89: result := FrmWearItem.ILabels[id - 60];
        90..119: result := FrmNewMagic.RiseLabels[id - 90];
        120..149: result := FrmNewMagic.MysteryLabels[id - 120];
    end;
end;

procedure TFrmAttrib.BLabel0MouseDown(Sender: TObject;
    Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
    tt: TGET_cmd;
    cnt: integer;
...
procedure TFrmAttrib.FormMouseMove(Sender: TObject; Shift: TShiftState; X,
    Y: Integer);
begin
    MouseInfoStr := '';
end;
...
procedure TFrmAttrib.listContentKeyUp(Sender: TObject; var Key: Word;
    Shift: TShiftState);
begin
    SAY_EdChatFrmBottomSetFocus
end;

procedure TFrmAttrib.A2ILabel0MouseUp(Sender: TObject;
    Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
    FITEMA2Hint.FVisible := false;
end;
...
begin
    ...
    if FrmAttrib.PanelBasic.Visible then exit;
...
begin
    ...
    if FrmAttrib.PanelMagic.Visible then exit;
    magicState(true);
...
begin                                                                           //$001DF833  绿    $0005A0EB 金黄     $00B1753F 蓝    $001A4DFB 红
    ...
    begin
        ...
        if rNameColor > 0 then result := result + '<' + inttostr(rNameColor) + '>';
        for i := 1 to rStarLevel do result := result + '☆';
        case rSex of
            1: result := result + format('%s(%s)', [rViewName, '男']);
            2: result := result + format('%s(%s)', [rViewName, '女']);
        else result := result + format('%s', [rViewName]);
        end;
        if rSetting.rsettingcount > 0 then result := result + format('<$0023A4FA>(%d孔)', [rSetting.rsettingcount]);
        if rSmithingLevel > 0 then result := result + format('<$0023A4FA>+%d', [rSmithingLevel]);
        result := result + #13#10;

        if rcreatename <> '' then result := result + '<$FF8000>〖' + rcreatename + '〗制造' + #13#10;

        if cItemTextClass.getText(rName) <> '' then
            result := result + cItemTextClass.getText(rName) + #13#10;
        if rGrade > 0 then result := result + format('品阶:%d', [rGrade]) + #13#10; //新-品介
        if rPrice > 0 then
        begin
            result := result + format('价格:%d', [rPrice]);

            if rPrice > 100000000 then result := result + format(' <$000080FF>(%.2f亿)', [rPrice / 100000000])
            else if rPrice > 10000 then result := result + format(' <$000080FF>(%.2f万)', [rPrice / 10000]);
            result := result + #13#10;                                          //价格
        end;
        if rCount > 1 then
        begin
            result := result + format('数量:%d', [rCount]);                     //数量
            if rCount > 100000000 then result := result + format(' <$000080FF>(%.2f亿)', [rCount / 100000000])
            else if rCount > 10000 then result := result + format(' <$000080FF>(%.2f万)', [rCount / 10000]);
            result := result + #13#10;
        end;
        if rSpecialLevel > 0 then
        begin
            result := result + '等级:' + Get10000To100(rSpecialLevel) + #13#10;
        end;
        if rKind = ITEM_KIND_35 then
        begin
         //   if rDurability > 0 then
            result := result + format('水量:%d/%d', [rCurDurability, rDurability]) + #13#10;
        end
        else
        begin
            if rDurability > 0 then result := result + format('持久:%d/%d', [rCurDurability, rDurability]) + #13#10;
        end;

        if boUpgrade then if MaxUpgrade > 0 then result := result + '最高精炼等级:' + inttostr(MaxUpgrade) + #13#10; //新 (最大升级别)
        if rboident then result := result + '可鉴定(通过鉴定可以得到更多属性)' + #13#10;

        astr := _get('', rLifeDataBasic);
        if astr <> '' then result := result + '〓基本〓' + #13#10 + astr;
        astr := _get('<$001A4DFB>', rLifeDataLevel);
        if astr <> '' then result := result + '<$001A4DFB>〓精炼〓' + #13#10 + astr;
        astr := _get('<$001DF833>', rLifeDataAttach);
        if astr <> '' then result := result + '<$001DF833>〓附加〓' + #13#10 + astr;
        astr := _get('<$00F7A61E>', rLifeDataSetting);
        if astr <> '' then result := result + '<$00F7A61E>〓镶嵌〓' + #13#10 + astr;
        if rSuitId > 0 then
        begin
            if WearitemClass.Wear.getSuitIDCount(rSuitId) >= 5 then
            begin
                astr := _get('<$001DF833>', rLifeDataSuit);
                if astr <> '' then result := result + '<$001DF833>〓套装〓' + #13#10 + astr;
            end else
            begin
                astr := _get('<$007B7B7B>', rLifeDataSuit);
                if astr <> '' then result := result + '<$007B7B7B>〓套装〓(穿戴全套激活属性)' + #13#10 + astr;
            end;
        end;
        ...
        begin
            if rSetting.rsettingcount >= 1 then
                if rSetting.rsetting1 <> '' then
                    result := result + '<$00F7A61E>镶嵌(1):' + rSetting.rsetting1 + #13#10;
            if rSetting.rsettingcount >= 2 then
                if rSetting.rsetting2 <> '' then
                    result := result + '<$00F7A61E>镶嵌(2):' + rSetting.rsetting2 + #13#10;
            if rSetting.rsettingcount >= 3 then
                if rSetting.rsetting3 <> '' then
                    result := result + '<$00F7A61E>镶嵌(3):' + rSetting.rsetting3 + #13#10;
            if rSetting.rsettingcount >= 4 then
                if rSetting.rsetting4 <> '' then
                    result := result + '<$00F7A61E>镶嵌(4):' + rSetting.rsetting4 + #13#10;

        end;
        case rlockState of
            1: result := result + '<$001A4DFB>锁定状态' + #13#10;
            2: result := result + format('<$001A4DFB>解锁时间已过:%.2f小时', [(rlocktime / 60)]) + #13#10;
        ...
        begin
            result := result + '<$0023A4FA>设计图纸/铸造模型' + #13#10;
            result := result + '<$0023A4FA>合成材料：';
            if rMaterial.NameArr[0] <> '' then result := result + format(' %s %d个', [rMaterial.NameArr[0], rMaterial.CountArr[0]]);
            if rMaterial.NameArr[1] <> '' then result := result + format(' %s %d个', [rMaterial.NameArr[1], rMaterial.CountArr[1]]);
            if rMaterial.NameArr[2] <> '' then result := result + format(' %s %d个', [rMaterial.NameArr[2], rMaterial.CountArr[2]]);
            if rMaterial.NameArr[3] <> '' then result := result + format(' %s %d个', [rMaterial.NameArr[3], rMaterial.CountArr[3]]);
            result := result + #13#10;
        end;
        ...
        begin
            case rSpecialKind of
                1: result := result + '<$001A4DFB>普通装备' + #13#10;
                2: result := result + '<$001A4DFB>生产装备' + #13#10;
                3: result := result + '<$001A4DFB>荣誉装备' + #13#10;
                4: result := result + '<$001A4DFB>合成装备' + #13#10;
                5: result := result + '<$001A4DFB>修炼装备' + #13#10;
                6: result := result + '<$001A4DFB>特殊道具(离开任务地图消失)' + #13#10;
            end;
        end;
//        if rboQuest then result := result + '<$001A4DFB>任务道具(离开任务地图消失)' + #13#10;
       // if rboPrestige then result := result + '<$0023A4FA>荣誉装备' + #13#10;
        ...
        if rboColoring then
            result := result + '允许染色' + #13#10;
        if rboTimeMode then
            result := result + '<$001A4DFB>有效时间:' + datetimetostr(rDateTime) + #13#10;
...
begin
    ...
    begin
        if assigned(FonUPdate) then FonUPdate(i, FdataArr[i]);
    end;
...
begin
    if (aindex < 0) or (aindex > high(FdataArr)) then exit;
    FdataArr[aindex].rboUser := false;
    if assigned(FonUPdate) then FonUPdate(aindex, FdataArr[aindex]);
end;
    ...
    begin
        if FdataArr[i].rboUser then
            if FdataArr[i].rkeyType = rkeyType then
                if FdataArr[i].rkey = rkey then UPdate(i, FdataArr[i].rtype, FdataArr[i].rkeyType, FdataArr[i].rkey);
    end;
end;
...
begin
    ...
    begin
        if FdataArr[i].rboUser then
            if FdataArr[i].rkeyType = rkeyType then
                if FdataArr[i].rkey = rkey then del(i);
    end;
end;
...
begin
    ...
    begin
        ...
        case adata.rkeyType of
            ...
            kcdk_HaveItem:
                begin
                    if (adata.rkey < 0) or (adata.rkey > 30) then exit;
...
procedure TFrmAttrib.onHaveitemQuestUPdate(akey: integer;
    aitem: titemdata);
begin
    FrmWearItem.onHaveitemQuestUPdate(akey, aitem);
end;

procedure TFrmAttrib.onHaveMysteryMagicUp(akey: integer;
    aitem: TMagicData);
begin
