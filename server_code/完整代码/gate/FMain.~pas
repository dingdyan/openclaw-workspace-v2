...
begin
    ...
    try
        ...
        ServerName := iniFile.ReadString('SERVER', 'SERVERNAME', '千年');
...
begin
   ...
   // if GameReceiver.ReceiveBufferCount > 0 then
    ...
    begin
        ...
        while LoginReceiver.Count > 0 do
        begin
...
begin
   ...
   // WriteLog(Socket.RemoteAddress + ' Error' + inttostr(ErrorCode));
    ErrorCode := 0;
    Socket.Close;
end;
...
begin
    ...
    begin
        ...
        begin
            WriteLog(format('TfrmMain.sckUserAcceptClientRead；ConnectorList.AddReceiveData失败', []));
            Socket.Close;
        end;
        exit;
    end;
...
begin
    ...
    begin
        ...
        if LoginSender.WriteAllow = true then
        begin
...
procedure TfrmMain.sckLoginConnectConnect(Sender: TObject;
    Socket: TCustomWinSocket);
begin
    LoginSender := TPacketSender.Create('Login', BufferSize_LOGIN_Send, Socket);
    LoginReceiver := TPacketReceiver.Create('Login', BufferSize_LOGIN_Rece);
...
begin
    ...
    begin
        ...
        if LoginReceiver.PutData(@buffer, nRead) = false then
        begin
