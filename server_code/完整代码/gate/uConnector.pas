...
uses
    FMain, uUtil, uGramerID, StrUtils;
...
//    PacketSender_r.Update(CurTick div 10);
 //   PacketReceiver_r.Update(CurTick div 10);
    ...
    begin
        if CreateTime + 1000 * 60 * 10 <= CurTick then
        begin
            ConnectorList.DeleteConnect(Socket);
        end;
    end;
...
begin
    ...
    begin
        ...
        begin
            ...
            if LoginData.CharInfo[i].boCHANGECharName then
            begin
                str := str + ':';
                Str := Str + '改名字';
            end;
                        ...
                        begin
                            ...
                            begin
                                ...
                                begin
                                    ...
                                    LGRecord.CharInfo[i].boCHANGECharName := false;
                                    if LoginSender <> nil then
                                    begin
                                        LoginSender.PutPacket(ConnectID, DB_LG_UPDATE, 0, @LGRecord, SizeOf(TLGRecord));
                                    end;
                                    exit;
                                end;
                            end;
                ...
                case FGameStatus of
                    gs_login:
                        begin
                            ...
                            //更新
                            if LoginSender <> nil then
                                LoginSender.PutPacket(ConnectID, DB_LG_UPDATE, 0, @LoginData, SizeOf(TLGRecord));
    ...
    case msg^ of
        ...
        CM_IDPASS:                                                              //客户端 发来 帐号密码
            begin
                ...
                if LoginSender <> nil then
                begin
                    LoginSender.PutPacket(ConnectID, DB_LG_SELECT, 0, @LGRecord.PrimaryKey, SizeOf(LGRecord.PrimaryKey));
                end;
            end;
        CM_CREATEIDPASS3:                                                       //创建 帐号
            begin
                ...
                if LoginSender <> nil then
                begin
                    LoginSender.PutPacket(ConnectID, DB_LG_INSERT, 0, @LGRecord, SizeOf(TLGRecord));
                end;
            ...
            case msg^ of
                ...
                CM_CREATECHAR:                                                  //创建 角色
                    begin
                        ...
                        ucharname := Trim((pcCreateChar^.rChar));
                        ...
                        if (not isFullHangul(ucharname)) then                   // or (not isGrammarID(ucharname)) then
                        begin
                            FGameStatus := gs_none;
                    ...
                    begin
                        ...
                        begin
                            ...
                            begin
                                ...
                                begin
                                    if LoginData.CharInfo[i].boCHANGECharName = false then
                                    begin
                                        FGameStatus := gs_none;
                        ...
                        if (not isFullHangul(rNewName)) then                    // or (not isGrammarID(ucharname)) then
                        begin
                            FGameStatus := gs_none;
