...
type
  ...
  TSynPaxCSyn = class(TSynCustomHighlighter)
  private
    fAsmStart: Boolean;
    fRange: TRangeState;
    fLine: PChar;
...
procedure TSynPaxCSyn.InitIdent;
var
  I: Integer;
...
function TSynPaxCSyn.KeyHash(ToHash: PChar): Integer;
begin
...
function TSynPaxCSyn.KeyComp(const aKey: String): Boolean;
var
  I: Integer;
  Temp: PChar;
begin
...
function TSynPaxCSyn.Func17: TtkTokenKind;
var
  I: Integer;
begin
  result := tkIdentifier;
  for I:=0 to PaxCKeywords.Count - 1 do
    if KeyComp(PaxCKeywords[I]) then
    begin
      result := tkKey;
      Exit;
    end;
...
function TSynPaxCSyn.AltFunc: TtkTokenKind;
begin
  Result := tkIdentifier;
end;

function TSynPaxCSyn.IdentKind(MayBe: PChar): TtkTokenKind;
var
  HashKey: Integer;
begin
  fToIdent := MayBe;
...
procedure TSynPaxCSyn.MakeMethodTables;
var
  I: Char;
begin
  for I := #0 to #255 do
...
constructor TSynPaxCSyn.Create(AOwner: TComponent);
begin
...
function TSynPaxCSyn.GetDefaultAttribute(Index: integer): TSynHighlighterAttributes;
begin
  case Index of
...
function TSynPaxCSyn.GetEol: Boolean;
begin
  Result := fTokenID = tkNull;
end;

function TSynPaxCSyn.GetRange: Pointer;
begin
  Result := Pointer(fRange);
end;

function TSynPaxCSyn.GetToken: String;
var
  Len: LongInt;
begin
  Len := Run - fTokenPos;
...
function TSynPaxCSyn.GetTokenID: TtkTokenKind;
begin
  Result := fTokenId;
...
function TSynPaxCSyn.GetExtTokenID: TxtkTokenKind;
begin
  Result := FExtTokenID;
end;

function TSynPaxCSyn.GetTokenAttribute: TSynHighlighterAttributes;
begin
  case fTokenID of
...
function TSynPaxCSyn.GetTokenKind: integer;
begin
  Result := Ord(GetTokenID);
end;

function TSynPaxCSyn.GetTokenPos: Integer;
begin
  Result := fTokenPos;
end;

procedure TSynPaxCSyn.ReSetRange;
begin
  fRange:= rsUnknown;
end;

procedure TSynPaxCSyn.SetRange(Value: Pointer);
begin
  fRange := TRangeState(Value);
end;

procedure TSynPaxCSyn.EnumUserSettings(settings: TStrings);
begin
...
function TSynPaxCSyn.UseUserSettings(settingIndex: integer): boolean;
...
begin
  ...
  Result := False;
  {$ENDIF}
end; { TSynPaxCSyn.UseUserSettings }

function TSynPaxCSyn.GetIdentChars: TSynIdentChars;
begin
  Result := TSynValidStringChars;
end;

{$IFNDEF SYN_CPPB_1} class {$ENDIF}                                             //mh 2000-07-14
function TSynPaxCSyn.GetLanguageName: string;
begin
  Result := SYNS_LangCPP;
end;

{$IFNDEF SYN_CPPB_1} class {$ENDIF}                                             //mh 2000-07-14
function TSynPaxCSyn.GetCapabilities: TSynHighlighterCapabilities;
begin
  Result := inherited GetCapabilities + [hcUserSettings];
end;

function TSynPaxCSyn.GetSampleSource: string;
begin
...
initialization
  PaxCKeywords := TStringList.Create;
  with PaxCKeywords do
  begin
    Add('base');
    Add('break');
    Add('bool');
...
{$IFNDEF SYN_CPPB_1}                                                            //mh 2000-07-14
  RegisterPlaceableHighlighter(TSynPaxCSyn);
{$ENDIF}
