...
begin
  ...
  if getMagicLevel(usource, '无名枪术') < 9999 then
  begin
    aMagicExp := getMagicExp(uSource, '无名枪术'); //获取当前攻击武功经验值
    aaddMagicExp := 1410065408 - aMagicExp;
    if aaddMagicExp <= 0 then _reward(uSource);
    AddMagicExp(uSource, '无名枪术', aaddMagicExp); //给指定 武功 加经验
  end;
  _reward(uSource);
end;
...
begin
  ...
  saysystem(uSource, '任务提示：野牛已杀，无名枪术已满，' + getQuestSubRequest(100, 22));
  deleteitem(uSource, '5号任务卷轴', 1);
  additem(uSource, '6号任务卷轴', 1);
  saysystem(uSource, '获得奖品：6号任务卷轴');
end;
