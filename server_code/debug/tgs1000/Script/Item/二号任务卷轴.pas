...
begin
  ...
  if getMagicLevel(usource, '无名剑法') < 9999 then
  begin
    aMagicExp := getMagicExp(uSource, '无名剑法'); //获取当前攻击武功经验值
    aaddMagicExp := 1410065408 - aMagicExp;
    if aaddMagicExp <= 0 then _reward(uSource);
    AddMagicExp(uSource, '无名剑法', aaddMagicExp); //给指定 武功 加经验
  end;
  _reward(uSource);
end;
...
begin
  ...
  saysystem(uSource, '任务提示：猪皮已获得，无名剑法已满，' + getQuestSubRequest(100, 10));
  deleteitem(uSource, '2号任务卷轴', 1);
  additem(uSource, '3号任务卷轴', 1);
  saysystem(uSource, '获得奖品：3号任务卷轴');
end;
