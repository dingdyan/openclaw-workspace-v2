...
begin
  ...
  if getMagicLevel(usource, '无名弓术') < 9999 then
  begin
    aMagicExp := getMagicExp(uSource, '无名弓术'); //获取当前攻击武功经验值
    aaddMagicExp := 1410065408 - aMagicExp;
    if aaddMagicExp <= 0 then _reward(uSource);
    AddMagicExp(uSource, '无名弓术', aaddMagicExp); //给指定 武功 加经验
  end;
  _reward(uSource);
end;
...
begin
  ...
  setQueststep(uSource, 26);
  saysystem(uSource, '任务提示：家传秘笈已获得，无名弓术已满，' + getQuestSubRequest(100, 26));
  deleteitem(uSource, '6号任务卷轴', 1);
  additem(uSource, '7号任务卷轴', 1);
  saysystem(uSource, '获得奖品：7号任务卷轴');
end;
